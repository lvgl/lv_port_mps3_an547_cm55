                            ...
                            arm_2d_scratch_mem_t *ptScratchMemory)
{
        ...
        {
            v_previous_init = vldrbq_gather_offset((uint8_t*)(&pwTarget[y*iTargetStride]), v_offset);
            v_previous  = vreinterpretq_u8(v_previous_init);
            v_previous = vshrq(v_previous, 1);
            for(int x = 1; x < iWidth; x++)
            {
                v_previous_init = vldrbq_gather_offset((uint8_t*)&pwTarget[y*iTargetStride+x], v_offset);
                ...
                vstrbq_scatter_offset((uint8_t*)&pwTarget[y*iTargetStride + x], v_offset, v_previous_init);
            }
        }
        if((iHeight&3) !=0)
        {
            ...
            v_previous_init = vldrbq_gather_offset_z((uint8_t*)&pwTarget[y*iTargetStride], v_offset, v_pred);
            v_previous  = vreinterpretq_u8(v_previous_init);
            v_previous = vshrq(v_previous, 1);
            for(int x = 1; x < iWidth; x++)
            {
                v_previous_init = vldrbq_gather_offset_z((uint8_t*)&pwTarget[y*iTargetStride+x], v_offset, v_pred);
                v_data = vreinterpretq_u8(v_previous_init);
                v_data = vshrq(v_data, 1);
                v_sub = vsubq(v_data, v_previous);
                v_sub = vqdmulhq(v_sub, v_ratio);
                ...
                v_sub = vshlq_n(v_previous, 1);
                v_previous_init = vreinterpretq_u32(v_sub);
                vstrbq_scatter_offset_p((uint8_t*)&pwTarget[y*iTargetStride + x], v_offset, v_previous_init, v_pred);
            }
        }
        #endif
        //column x4
